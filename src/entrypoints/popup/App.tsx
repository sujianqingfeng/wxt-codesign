...
function App() {
		...
		const loadFromStorage = async () => {
			try {
				...
				if (
					!activeTab ||
					!activeTab.url ||
					!activeTab.url.includes("codesign.qq.com/app/design/")
				) {
				...
				const storageKey =
					`session:codesign_screens_${currentDesignId}` as const
				const storedData = await storage.getItem<{
					screens: Screen[]
					timestamp: string
				}>(storageKey)

				if (storedData?.screens && storedData.screens.length > 0) {
					...
					showToast(
						`Loaded ${storedData.screens.length} screens from cache`,
						"info",
					)
				} else {
		...
		try {
			...
			if (result.success && result.data) {
				...
				if (result.designId) {
					...
					const storageKey =
						`session:codesign_screens_${result.designId}` as const
				...
				if (screensData.length > 0) {
					showToast(
						`Successfully loaded ${screensData.length} screens`,
						"success",
					)
				} else {
	...
	const handleScreenClick = async (screen: Screen) => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Meta data request failed with status: ${response.status}`,
				)
			}
			...
			showToast(
				`Error fetching meta data: ${err instanceof Error ? err.message : "Unknown error"}`,
				"error",
			)
		} finally {
	...
	const copyMetaData = () => {
		...
		navigator.clipboard
			...
			.then(() => {
				showToast(
					`Meta data for "${selectedScreen.name}" copied to clipboard!`,
					"success",
				)
			})
			.catch((err) => {
				...
				showToast(
					`Failed to copy meta data: ${err instanceof Error ? err.message : "Unknown error"}`,
					"error",
				)
			})
	}
	...
	const getImageUrl = useCallback((screen: Screen): string => {
		if (screen.full_preview_url) {
			return screen.full_preview_url
		}

		if (screen.thumbnail_url) {
						...
						{data.map((item, index) => {
							...
							const itemKey =
								typeof item === "object" && item !== null
									? `item-${Object.keys(item).join("-")}-${index}`
									: `item-${String(item).substring(0, 10)}-${index}`
	...
	const preloadImage = useCallback((src: string, id: string) => {
		if (!src) return
	...
	}, [screens, getImageUrl, preloadImage])
				...
				<div className="bg-white rounded-lg p-5 mt-6 shadow-md border border-slate-200">
					<div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-200">
						<h3 className="text-base font-semibold text-slate-800">
							Meta Data for: {selectedScreen.name}
						</h3>
						<button
							type="button"
						...
						{screens.map((screen) => (
							...
							>
								<div className="relative aspect-video bg-slate-100 group">
									...
									{screen.meta_url && (
										<div className="absolute top-2 right-2 w-2.5 h-2.5 bg-emerald-500 rounded-full border-2 border-white shadow-sm" />
									)}
								</div>
								<div className="p-3">
			...
			{!error && !designId && !loading && (
				<div className="text-center py-10">
					...
					<p className="text-slate-500 mb-2">
						Navigate to a Codesign design page
					</p>
					<p className="text-slate-400 text-sm">
						Then click "Sync Screens" to fetch data
					</p>
				</div>
			)}
		</div>
